...
const { ipcRenderer, Menu, ExifImage, fs, utils, controls, options, config } = window;
      ...
      submenu: [
        {
          ...
          click: () => {
            ipcRenderer.send('open-directories-dialog');
          }
        },
        {
          label: 'shuffle',
          click: () => {
            ipcRenderer.send('shuffle-files');
          }
        },
        {
          label: 'options',
      ...
      submenu: [
        {
          ...
          click: () => {
            ipcRenderer.send('open-url-in-external', 'https://github.com/eddysant/photo-slap');
          }
        },
        {
          label: 'report issue',
          click: () => {
            ipcRenderer.send('open-url-in-external', 'https://github.com/eddysant/photo-slap/issues');
          }
        }
      ]
    }
...
ipcRenderer.on('update-display-image', (e, filename) => {
...
ipcRenderer.on('get-files', (e, opened_directories) => {
  ...
  utils.getFiles(opened_directories, options.getIncludeVideos(), (files) => {
    ...
    ipcRenderer.send('load-files', files);
